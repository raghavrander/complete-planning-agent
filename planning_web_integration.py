...
def run_research_agent(user_question: str, context: dict) -> str:
    ...
    """
    try:
        response = client.chat.completions.create(messages=[{"role": "user", "content": research_prompt}], model="openai/gpt-oss-120b")
...
def run_planning_agent(user_prompt: str) -> dict:
    ...
    **Example for "Plan my day":** {{"plan": [{{"step_description": "First, let's get some breakfast.", "tool_query": "breakfast restaurant"}}, {{"step_description": "Next, a fun morning activity.", "tool_query": "museum or park"}}, {{"step_description": "Time for lunch!", "tool_query": "lunch restaurant"}}]}}
    """
    try:
        plan_completion = client.chat.completions.create(messages=[{"role": "system", "content": "You are an expert travel planner that only outputs JSON."}, {"role": "user", "content": plan_prompt}], model="openai/gpt-oss-120b", temperature=0.1, response_format={"type": "json_object"})
...
def synthesize_itinerary(itinerary_items: list) -> str:
    ...
    try:
        synthesis_completion = client.chat.completions.create(messages=[{"role": "user", "content": synthesis_prompt}], model="openai/gpt-oss-120b", temperature=0.5)
...
def run_conversational_agent(user_prompt: str) -> str:
    ...
    """
    try:
        response = client.chat.completions.create(messages=[{"role": "user", "content": conversational_prompt}], model="openai/gpt-oss-120b", temperature=0.6)
        return response.choices[0].message.content
    except Exception as e:
...
def run_router_agent(user_prompt: str) -> str:
    ...
    User request: "{user_prompt}" """
    try:
        router_completion = client.chat.completions.create(messages=[{"role": "system", "content": "You are an intent classification agent that only responds with a single word: 'planner', 'finder', 'research', or 'conversational'."}, {"role": "user", "content": router_prompt}], model="openai/gpt-oss-20b", temperature=0.0)
